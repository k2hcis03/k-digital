...
struct recipe9_1{
	...
	struct hrtimer recipe9_1_timer;
	int timer_period;
	char name[100];
	struct gpio_desc *gpio;
};

enum hrtimer_restart recipe9_1_hr_timer(struct hrtimer *t)
{
	static int toggle = 0;

	struct recipe9_1*recipe_private = from_timer(recipe_private, t, recipe9_1_timer);
	struct device *dev;
	ktime_t ktime;

	dev = &recipe_private->pdev->dev;
//	dev_info(dev, "recipe9_1_hr_timer enter %d\n", recipe_private->timer_period);
	...
	hrtimer_forward(&recipe_private->recipe9_1_timer,
			hrtimer_cb_get_time(&recipe_private->recipe9_1_timer), ktime);
...
{
	struct recipe9_1*recipe_private = dev_get_drvdata(dev);
...
{
	struct recipe9_1* recipe_private;
	struct device *dev;
	recipe_private = container_of(file->private_data, struct recipe9_1, recipe_miscdevice);
	dev = &recipe_private->pdev->dev;
...
static int recipe_probe(struct platform_device *pdev)
{
	...
	recipe_private = devm_kzalloc(&pdev->dev, sizeof(struct recipe9_1), GFP_KERNEL);
	recipe_private->pdev = pdev;
	recipe_private->timer_period = 200;

	ktime = ktime_set( 0, US_TO_NS(recipe_private->timer_period));
	hrtimer_init(&recipe_private->recipe9_1_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL );
	recipe_private->recipe9_1_timer.function = &recipe9_1_hr_timer;
	hrtimer_start(&recipe_private->recipe9_1_timer, ktime, HRTIMER_MODE_REL );
...
{
	struct device *dev = &pdev->dev;
	struct recipe9_1*recipe_private = platform_get_drvdata(pdev);
	int ret;

	dev_info(dev, "recipe_remove() function is called.\n");

	ret = hrtimer_cancel(&recipe_private->recipe9_1_timer);
	if(ret){
